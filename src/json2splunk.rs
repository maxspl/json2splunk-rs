...
const INGESTION_METADATA_SOURCE: &str = "json2splunk:ingestion_metadata";
const INGESTION_METADATA_SOURCETYPE: &str = "_json";
...
impl Json2Splunk {
    ...
    fn run_parallel_line_pipeline(&self, 
        ...
        // If Splunk HEC is configured, send a metadata event with the expected count.
        if let Some(ref hec_template) = hec_template {
            let total = event_count.load(Ordering::Relaxed);
            let payload = Json2Splunk::build_ingestion_metadata_payload(total, ctx.as_ref());
    ...
    fn run_parallel_csv_pipeline(
        ...
        // Emit a metadata event with the expected number of events for this CSV file.
        if let Some(ref hec_template) = hec_template {
            let total = event_count.load(Ordering::Relaxed);
            let payload = Json2Splunk::build_ingestion_metadata_payload(total, ctx.as_ref());
            let mut hec = hec_template.clone();
    ...
    fn build_ingestion_metadata_payload(expected_event_count: u64, ctx: &EventContext) -> Value {
        ...
        // Metadata events are generated by json2splunk-rs itself. Keep them in a
        // dedicated Splunk source instead of mixing them with forensic events from
        // the original source.
        meta_ctx.source = INGESTION_METADATA_SOURCE.to_string();
        meta_ctx.sourcetype = INGESTION_METADATA_SOURCETYPE.to_string();

        // The metadata record does not contain the original event timestamp/host
        // fields, so use ingestion time and the rule/file-derived host.
        meta_ctx.timestamp_paths.clear();
        meta_ctx.host_path = None;

        let record = json!({
            "event_type": "ingestion_metadata",
            "expected_event_count": expected_event_count,
            "original_source": ctx.source.clone(),
            "original_sourcetype": ctx.sourcetype.clone(),
            "original_sourcefile": ctx.sourcefile.clone(),
        });

        // Do not add a normal event uid to metadata records. The uid field is meant
        // for real indexed events, for example when flagging timeline entries.
        Self::build_payload(record, &meta_ctx, false)
    }
